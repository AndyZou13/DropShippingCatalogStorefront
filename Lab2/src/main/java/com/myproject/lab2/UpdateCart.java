...
public class UpdateCart extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        for(Item item : items){
            String op = request.getParameter(item.UPC);
            if(op == null) continue;
            
            if(op.equals("Add")){
                CART_CRUD.create(SessionInfo.userInfo.getUsername(), item.getUPC());
                SessionInfo.userInfo.getCart().add(item);
            }else if(op.equals("Remove")){
                CART_CRUD.delete(SessionInfo.userInfo.getUsername(), item.getUPC());
                SessionInfo.userInfo.getCart().remove(item);
            }
